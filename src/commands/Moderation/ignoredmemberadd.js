module.exports = {
  ...
  run: async (client, message, args, settings) => {
    ...
    if (member.hasPermission("ADMINISTRATOR")) return message.reply(`commande interdite, ce membre est administrateur`);
    const dbMember = await client.getDbMember(member, message.guild);
    if (dbMember.isIgnored == true) return message.reply(`le membre ${member} est déjà ignoré`);
    else await client.updateMember(member, message.guild, "isIgnored", true);
    return message.channel.send(`${client.emotes.check} Le membre ${member} sera ignoré`);
  },
  cooldown: 5,
