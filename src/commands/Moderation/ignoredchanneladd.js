module.exports = {
  ...
  run: async (client, message, args, settings) => {
    ...
    if (settings.modules.moderation.ignoredChannels.includes(channel.id)) return message.reply(`le channel ${channel} est déjà ignoré`);
    else {
      await client.addIgnoredChannel(message.guild, channel.id);
      return message.channel.send(`${client.emotes.check} Les commandes dans le channel ${channel} seront ignorées`);
    }
  },
  cooldown: 5,
