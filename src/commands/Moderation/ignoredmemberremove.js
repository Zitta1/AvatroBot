module.exports = {
  ...
  run: async (client, message, args, settings) => {
    ...
    if (dbMember.isIgnored == false) return message.reply(`le membre ${member} n'est pas déjà ignoré`);
    else await client.updateMember(member, message.guild, "isIgnored", false);
    return message.channel.send(`${client.emotes.check} Le membre ${member} ne sera plus ignoré`);
  },
  cooldown: 5,
